...
> | **[第 2 篇](第2篇-BC28联网与AT指令实测.md)** | **BC28 联网 + AT 指令实测**（含透传固件） | ✅ **已发布** |
...
| BC28 联网（登录网络 + TCP 收发） | ✅ **已验证**，见 [第 2 篇](第2篇-BC28联网与AT指令实测.md) |
| 墨水屏 | ⚠️ 未验证，第 4 篇的事 |
...
| **③ 厂家资料包**（`<资料包>`） | §5.2 换例程、第 2–4 篇要的 13 个例程工程，都在里面。**只能跟卖家/作者要**，3.4 G 传不上仓库 |
...
| **⑥ 第 2 篇**（仓库根目录 `第2篇-BC28联网与AT指令实测.md`） | 本篇的续章：**BC28 联网 + AT 指令实测表**。**本篇自己不含任何 NB-IoT 内容**，要联网就看它；配套的透传固件在 `04-AT透传/` |

> ①②④⑤⑥ 这几样**都已经在这个仓库里**，读者 clone 或点 `Code → Download ZIP` 就全拿到了。
> 只有 **③ 厂家资料包**（3.4 G）没法放进来，需要单独向卖家/作者索要 —— 它**不是跑通本篇的必需品**，
> 但第 2–4 篇要用的 13 个例程工程（建透传固件要的例程 1、墨水屏资料等）都在里面，建议一开始就一起要过来。
...
| **QNavigator**（移远官方工具） | 资料包里 `10.NBIOT专用调试工具`。**第 2 篇最终没用它** —— 走的是"自己烧一份透传固件、用 PC 直接敲 AT"这条路（更底层，也更好复现）。它能省事，想用就用 |
...
| BC28 联网 | ✅ 已验证，见 [第 2 篇](第2篇-BC28联网与AT指令实测.md) |
| 墨水屏 | ⚠️ 完全未验证，第 4 篇的事 |
