...
| ③ | **本套脚本** = `<工具目录>` | **跟本篇文档一起拿**（作者另外发给你）。没有它，§3 的"双击"跑不起来，得自己敲 Keil 命令行 | 每个操作都退化成手敲命令 |
...
**脚本里不写死本机路径**：Keil 装在哪儿、烧写算法在哪儿都是自动探测的，所以可以整份拷到别的电脑用。
...
|---|---|
| **① 本篇文档** | 就是这一份 |
| **② 本套脚本**（`<工具目录>`） | 没有它 §3 的"双击"跑不起来（见 §2.0 ③） |
| **③ 厂家资料包**（`<资料包>`） | §5.2 换例程、§9.2 第 2 篇要的 QNavigator，都在里面 |
| **④ Hello World 的源码 `main.c`**（本仓库里的 `01-环境搭建/main.c`） | §4 那三段日志就是它跑出来的。**想自己造一个工程**：按 §5.2 把资料包的**例程 1** 拷到短路径、跑一遍 `prep_project.py`，再把这个 `main.c` 覆盖到 `Project\Test\main.c` 即可 |

> 作者机器上还有两份**写给自己看的**笔记：工具包里的 `README.md`（"换一台电脑怎么办"）和工程里的
> `README.md`（源码结构、串口参数细节）。**那两份写的是作者本机的路径**，直接发出来会有对不上的地方 ——
> 本篇已经把其中对读者有用的部分都收进来了，所以**发这一份就够**。
